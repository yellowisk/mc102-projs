...
class Weights():
    """
    Decidimos em nosso projeto adicionar um valor numérico a cada peça de dominó em nosa mão para depois \n
    -na função general e sabotage- para isso, criamos uma classe Weights em que cada jogador terá seu \n
    próprio objeto dessa classe e dentro da classe também estão as funções utilizadas para aumentar tal \n
    valor numérico.
    """
    ...
    def general(self, tiles, extremos):
        ...
        if len(extremos) == 0:
            ...
            if pesos.count(maior) > 1:
                for i in range(pesos.count(maior)):
                    if playable[pesos.index(maior)][2] > best:
                        best = playable[pesos.index(maior)][2]
                        maior_soma = (playable[pesos.index(maior)][0], playable[pesos.index(maior)][1])
                    pesos[pesos.index(maior)] -= 1
                return maior_soma
            else:
                return (playable[pesos.index(maior)][0], playable[pesos.index(maior)][1])
...
class Jogador2(Player):
    ...
    def __init__(self):
        super().__init__(281801, "Dor", "img/dor.gif")
...
class Jogador1(Player):
    ...
    def __init__(self):
        super().__init__(245293, "Tristeza", "img/tristeza.gif")
...
def pair_name():
    return "Aflição" # Defina aqui o nome da sua dupla
